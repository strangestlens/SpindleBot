# Archived: the original 6-phase roadmap

*Written early 2026. Superseded — kept for context, not as a plan of record.*

This was SpindleBot's first roadmap, numbered Phases 1–6. Two of those phases
shipped and the rest were overtaken by a different design:

- **Phase 1 (Config & portability)** and **Phase 2 (Modular architecture +
  testing)** are **complete**. All import logic is a tested Python package.
- Everything after that was superseded by the **content-addressed library
  refactor** — a SpindleBot-owned SQLite DB tracking content identity and every
  copy's location — which uses its own phase labels (A, 0, 1, 2, 3…). Don't
  conflate the two numbering schemes.
- The parts still genuinely forward-looking (remote access, the Mac app, a
  plain-text lyric source, the collection-audit extensions) were carried over to
  the live [`ROADMAP.md`](../../ROADMAP.md).

Two sketches below never shipped as written, and are annotated inline where they
appear: the Phase 1 config block (live keys differ) and the Phase 2 `lyrics/`
subpackage (alignment was built outside `spindlebot/` as the peer
`lyric_timing/` package, because torch and demucs have no business in the core
pipeline or in CI).

For current state see [`CLAUDE.md`](../../CLAUDE.md) and
