...
def test_register_explicit_location(conn, tmp_path):
    loc = LocationConfig(name="DwRugged", kind="local_drive",
    ...
    assert rugged.kind == "local_drive"
...
def test_disabled_entries_are_skipped(conn, tmp_path):
    loc = LocationConfig(name="Off", kind="local_drive", enabled=False)
